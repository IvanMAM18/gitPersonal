...
import useNegocioDetalles from "../utils/hooks/useNegocioDetalles";
...
function MiNegocioDetalles() {
    const [negocio, updateNegocio] = useNegocioDetalles();
    const [tramiteId, setTramiteId] = useState(null);

    const reload = () => {
        const routeArray = location.href.split("/");
        const negocioId = routeArray[routeArray.length - 1];
        updateNegocio(negocioId);
    };
    ...
    return (
        <div className="sare--container">
            ...
            <Modal
                destroyOnClose
                open={tramiteId !== null}
                onCancel={() => setTramiteId(null)}
                onOk={() =>setTramiteId(null)}
                footer={[
            ...
            >
                ...
                <br />
                { (negocio?.revisiones?.length === 0) && 'Estamos revisando su tramite....'}
                <div></div>
                <Timeline>
