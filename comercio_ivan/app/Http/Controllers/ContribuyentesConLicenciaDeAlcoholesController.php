...
class ContribuyentesConLicenciaDeAlcoholesController extends Controller
{
    public function index($year)
    {
        ...
        ->whereHas('revisiones', fn($query) => $query->whereHas('requirements', fn($query) => $query->where('requisitos.id', Requisito::$NUMERO_DE_LICENCIA_DE_ALCOHOL)->aprobado()))
        ->whereYear('created_at', $year)
        ->with([
            'tramitable',
            'revisiones' => function ($query) {
                $query->whereHas('requirements', fn($query) => $query->where('requisitos.id', Requisito::$NUMERO_DE_LICENCIA_DE_ALCOHOL))
